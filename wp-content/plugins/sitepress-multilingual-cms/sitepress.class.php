...
class SitePress
{
	...
	function __construct() {
		...
		// Post/page language box
			if ( $pagenow === 'post.php' || $pagenow === 'post-new.php' || $pagenow === 'edit.php' || defined( 'DOING_AJAX' ) ) {
				...
				$post_edit_screen = new WPML_Post_Edit_Screen( $this->get_setting( 'language_negotiation_type' ) );
	...
	public function api_filters() {
		...
		add_filter( 'wmpl_get_setting', array( $this, 'filter_get_setting' ), 10, 2 );
		add_filter( 'wmpl_get_current_language', array( $this, 'get_current_language_filter' ), 10, 0 );
		add_filter( 'wmpl_get_user_admin_language', array( $this, 'get_user_admin_language_filter' ), 10, 2 );
		add_filter( 'wmpl_is_admin_action_from_referer', array( $this, 'check_if_admin_action_from_referer' ), 10, 0 );
		add_filter( 'wmpl_current_user', array( $this, 'get_current_user' ), 10, 0 );
		...
		if (!empty( $this->settings[ 'existing_content_language_verified' ] ) && (!is_admin() || SitePress::check_settings_integrity()) ) {
			...
			if (! defined( 'WP_ADMIN' ) ) {
				...
				switch ( $this->settings[ 'language_negotiation_type' ] ) {
					case 1:
						$language_part = $wpml_url_converter->get_language_from_url( $_SERVER[ "REQUEST_URI" ]);
						if ( in_array( $language_part, $active_languages ) ) {
							$this->this_lang = $language_part;
						...
						} elseif ( in_array( $language_part, $this->get_setting( 'hidden_languages', array() ) ) ) {
						...
						if ( $this->settings[ 'language_negotiation_type' ] == 1
                             && $this->settings[ 'urls' ][ 'directory_for_default_language' ]
                             && in_array( $this->settings[ 'urls' ][ 'show_on_root' ],
                                          array( 'page', 'html_file' ) ) ) {
					...
					case 2:
						...
						$redirect_helper->maybe_redirect ( $this->get_setting ( 'language_domains' ) );
						break;
					case 3:
					default:
					/** @var WPML_Lang_Parameter_Converter $wpml_url_converter */
					$get_lang        = $wpml_url_converter->lang_by_get_params ();
					$this->this_lang = $get_lang;
					require ICL_PLUGIN_PATH . '/inc/request-handling/redirection/wpml-redirect-by-param.class.php';
					$redirect_helper = new WPML_Redirect_By_Param( $this->get_setting ( 'taxonomies_sync_option', array() ), $get_lang );
					$redirect_helper->maybe_redirect ();
				}
	...
	function set_term_filters_and_hooks(){
		...
		add_action( 'deleted_term_relationships', array( 'WPML_Terms_Translations', 'deleted_term_relationships' ), 10, 2 );
    ...
    function preserve_lang_param_after_bulk_category_delete( $location ) {
        $get_lang = filter_input ( INPUT_GET, 'lang', FILTER_SANITIZE_FULL_SPECIAL_CHARS );
	...
	function set_admin_language()
	{
		...
		$this->admin_language = $this->user_lang_by_authcookie();
	...
	function set_active_languages( $arr ) {
		...
		$this->active_languages = $this->get_active_languages ( true );
    ...
    function get_languages( $lang = false, $active_only = false, $refresh = false ) {
        global $wpdb;
        if ( !$lang ) {
        ...
        $res_query_prepared = $wpdb->prepare( $res_query, $lang, $lang );
        $res = $wpdb->get_results( $res_query_prepared, ARRAY_A );
        $languages = array();

        foreach ( (array) $res as $r ) {
            $languages[ $r[ 'code' ] ] = $r;
            $this->icl_language_name_cache->set( 'language_details_' . $r[ 'code' ] . $lang, $r );
        }

        if ( $active_only ) {
            $this->icl_language_name_cache->set( 'in_language_' . $lang, $languages );
        }
	...
	function get_language_code( $english_name ) {
		global $wpdb;
		$query  = $wpdb->prepare( " SELECT code FROM {$wpdb->prefix}icl_languages WHERE english_name = %s LIMIT 1", $english_name );
	...
	function get_current_language() {
		...
		$this->this_lang = $this->this_lang ? $this->this_lang : $wpml_url_converter->get_language_from_url( $_SERVER[ "REQUEST_URI" ]);
	...
	function js_load()
	{
		if ( is_admin() && !defined( 'DOING_AJAX' ) ) {
			...
			$page_basename         = $page === null ? false : preg_replace ( '/[^\w-]/', '', str_replace ( '.php', '', $page ) );
			$this->scripts_handler = new WPML_Admin_Scripts_Setup( $this->get_current_language (), $this->get_default_language (), $page_basename );
	...
	function delete_element_translation( $trid, $element_type, $language_code = false ) {
		...
		if ( $trid !== false && is_numeric( $trid ) && $element_type !== false && is_string( $trid ) ) {
			...
			$result = $wpdb->delete( $wpdb->prefix . 'icl_translations', $delete_where, $delete_formats );
			$this->icl_translations_cache->clear();
		}
	...
	static function get_source_language_from_referer() {
		...
		$source_lang = isset( $query_parts[ 'lang' ] ) ? $query_parts[ 'lang' ] : false;
		...
		if ( $sitepress_settings[ 'sync_delete' ] != 1 ) {
			...
			if ( $trid ) {
				...
				foreach ( $translations as $translation ) {
					//TODO: [WPML 3.3] check that the post exists too
	...
	public function _sync_custom_field( $post_id_from, $post_id_to, $meta_key ) {
		...
		$values_from = $wpdb->get_col ( $wpdb->prepare ( $sql, array( $post_id_from, $meta_key ) ) );
		$values_to   = $wpdb->get_col ( $wpdb->prepare ( $sql, array( $post_id_to, $meta_key ) ) );

		$removed = array_diff( $values_to, $values_from );
		foreach ( $removed as $v ) {
			$delete_prepared = $wpdb->prepare( "DELETE FROM {$wpdb->postmeta}
												WHERE post_id=%d
												AND meta_key=%s
												AND meta_value=%s",
		...
		foreach ( $added as $v ) {
			$insert_prepared = $wpdb->prepare( "INSERT INTO {$wpdb->postmeta}(post_id, meta_key, meta_value)
												VALUES(%d, %s, %s)",
			                                   array($post_id_to, $meta_key, $v) );
	...
	function get_element_translations( $trid, $el_type = 'post_post', $skip_empty = false, $all_statuses = false, $skip_cache = false )
	{
		...
		$temp_elements = wp_cache_get($cache_key, $cache_group, false, $cache_found);
	...
	function get_element_trid( $element_id, $el_type = 'post_post' ) {
		...
		} else if ( strpos ( $el_type, 'post_' ) === 0 ) {
			global $wpml_post_translations;
	...
	/**
	 * @param int    $element_id Use term_taxonomy_id for taxonomies, post_id for posts
	 * @param string $element_type Use comment, post, page, {custom post time name}, nav_menu, nav_menu_item, category, post_tag, etc. (prefixed with 'post_', 'tax_', or nothing for 'comment')
...
*@return null|string
	...
	function get_language_for_element( $element_id, $element_type = 'post_post' ) {
		...
		$language_for_element_prepared = $wpdb->prepare( "SELECT language_code FROM {$wpdb->prefix}icl_translations WHERE element_id=%d AND element_type=%s", array( $element_id, $element_type ) );
	...
	function language_filter() {
		...
		$post_lang_filter = new WPML_Post_Language_Filter( $this->get_default_language () );
		$post_lang_filter->post_language_filter ();
	}
	...
	function exclude_other_language_pages2( $arr )
	{
		...
		if ( $current_language != 'all' ) {
			...
			$cache_key = md5(wp_json_encode($new_arr));
	...
	public function create_term( $cat_id, $tt_id, $taxonomy ) {
		$term_actions = $this->get_term_actions();
		$term_actions->save_term_actions($cat_id, $tt_id, $taxonomy);
	}

	private function get_term_actions() {
	...
	function terms_clauses( $clauses, $taxonomies, $args ) {
		...
		$where_lang = $lang === 'all' ? '' : $wpdb->prepare( " AND icl_t.language_code = %s ", $lang );

		$clauses[ 'join' ] .= " LEFT JOIN {$wpdb->prefix}icl_translations icl_t
                                    ON icl_t.element_id = tt.term_taxonomy_id
                                        AND icl_t.element_type = CONCAT('tax_', tt.taxonomy)";
		...
		foreach ( $w_active_languages as $k => $lang ) {
			...
			if ( is_singular()
				...
				if ( isset( $translations[ $lang[ 'code' ] ] ) ) {
					...
					$lang[ 'translated_url' ] = get_term_link( (int)$translations[ $lang[ 'code' ] ]->term_id, $taxonomy );
					$icl_adjust_id_url_filter_off = false; // restore default behaviour
					$lang[ 'missing' ]            = 0;
				} else {
					if ( $icl_lso_link_empty ) {
						if ( !empty( $template_args[ 'link_empty_to' ] ) ) {
					...
					} elseif($current_language !== $lang[ 'code' ] ) {
						// don't skip the current language
						$skip_lang = true;
					}
					$lang[ 'missing' ] = 1;
				}
	...
	function get_desktop_language_selector() {
		...
		$language_selector = '<div id="lang_sel" '
                             ...
                             .$class_icl_rtl.' ><ul><li><a href="#" class="lang_sel_sel icl-'.$w_this_lang['code'].'">';
    ...
    private function get_ls_settings( $lang, $lang_native_hidden = false, $lang_translated_hidden = false ) {
        ...
        if ( $icl_language_switcher_preview ) {
            ...
            $lang_native_hidden = !$this->settings[ 'icl_lso_native_lang' ];
            $lang_translated = $lang[ 'translated_name' ];
            $lang_translated_hidden = !$this->settings[ 'icl_lso_display_lang' ];
        } else {
	...
	function get_mobile_language_selector() {
		...
		$language_selector_mobile = '<div id="lang_sel_click" onclick="wpml_language_selector_click.toggle();"
	...
	function get_language_selector() {
		...
		if ( !function_exists( 'wpml_home_url_ls_hide_check' ) || !wpml_home_url_ls_hide_check() ) {
			...
			$mobile = $this->settings[ 'icl_lang_sel_stype' ] === 'dropdown'
					  && ( $type === 'mobile'
						   || ( $type === 'mobile-auto'
			...
			return $mobile === true ? $this->get_mobile_language_selector()
	...
	function pre_option_home( $setting = false )
	{
		...
		$ret = $is_template_file && $is_direct_call ? $this->language_url( $this->this_lang ) : $setting;
    ...
    private function adjust_query_ids($q, $index){
        if ( !empty( $q->query_vars[ $index] ) ) {
            $untranslated          = is_array( $q->query_vars[ $index ] ) ? $q->query_vars[ $index ] : explode( ',', $q->query_vars[ $index] );
			
			/** @var WPML_Post_Translation $wpml_post_translations */
			global $wpml_post_translations;
			$wpml_post_translations->prefetch_ids($untranslated);
			
	        $ulanguage_code = $this->get_current_language ();
			
            $translated = array();
            foreach ( $untranslated as $element_id ) {
                $translated[ ] = $wpml_post_translations->element_id_in( $element_id, $ulanguage_code );
            }
	...
	function locale()
	{
		...
		if(!$l) {
			if (defined( 'WP_ADMIN' ) ) {
				$l = $this->get_locale( $this->user_lang_by_authcookie() );
				} else {
	...
	function pre_option_page_on_front() {
		...
		return $this->pre_option_page ( 'page_on_front' );
	}
	...
	function pre_option_page_for_posts() {
		...
		return $this->pre_option_page ( 'page_for_posts' );
	}
		...
		if ( $cached_edit_post_link ) {
			...
			if ( current_user_can( 'edit_post', $id ) ) {
				...
				if ( $id ) {
					...
					$link .= $and . 'lang=' . $wpml_post_translations->get_element_lang_code($id);
				}
			}
	...
	function option_sticky_posts( $posts ) {
		...
		if ( $wp_query && $wp_query->in_the_loop ) {
			remove_filter( 'pre_option_sticky_posts', array( $this, 'option_sticky_posts' ) );
			$posts = $posts ? $posts : get_option( 'sticky_posts' );
			add_filter( 'pre_option_sticky_posts', array( $this, 'option_sticky_posts' ), 10, 2 );

			if ( $posts ) {
				$posts_in = wpml_prepare_in( $posts, '%d' );
				$posts    = $wpdb->get_col( $wpdb->prepare( "SELECT element_id
														FROM {$wpdb->prefix}icl_translations
														WHERE element_type LIKE 'post%%'
															AND element_id IN ({$posts_in})
															AND language_code = %s",
				                                            $this->this_lang ) );
			}
	...
	function show_user_options()
	{
		...
		$all_languages = $this->get_languages( $user_language ? $user_language : $this->settings[ 'admin_default_language' ] );
	...
	function add_posts_management_column( $columns ) {
		...
		if ( is_null( $__management_columns_posts_translations ) ) {
			...
			$__management_columns_posts_translations = new WPML_Post_Status_Display( $post_ids );
		}
		$active_languages = $this->get_active_languages();
		$languages        = array();
		...
		foreach ( $columns as $k => $v ) {
			$new_columns[ $k ] = $v;
			if ( $k == 'title' || $k == 'name' ) {
				if ( !isset($new_columns[ 'icl_translations' ]) ) {
	...
	function add_content_for_posts_management_column( $column_name ) {
		...
		foreach ( $active_languages as $v ) {
			...
			$icon_html = $__management_columns_posts_translations->get_status_html ( $id, $v[ 'code' ] );
			echo $icon_html;
		}
	}
	...
	function xmlrpc_call_actions( $action ) {
		...
		$params = icl_xml2array ( print_r ( readfile ( 'php://input' ), true ) );
	...
	function set_language_cookie() {
		...
		$wpml_request_handler->set_language_cookie($this->get_current_language());
	}
	...
	function is_translated_post_type( $type ) {
		$option_key          = 'custom_posts_sync_option';
		$readonly_config_key = 'custom-types_readonly_config';

		return $this->is_translated_element( $type, $option_key, $readonly_config_key, $this->get_always_translatable_post_types() );
	}
	...
	function verify_post_translations( $post_type ) {
		...
		$sql          = "
				...
				";
		$sql_prepared = $wpdb->prepare( $sql, array( $post_type ) );
		$results      = $wpdb->get_col( $sql_prepared );
	...
	function slug_template($template){
		...
		if (!$is_taxonomy || $this->is_translated_taxonomy( $taxonomy ) && $current_language != $default_language ) {
			...
			if ( $current_term ) {
				$taxonomy_prefix = $taxonomy == 'post_tag' ? 'tag' : $taxonomy;
				$templates[ ] = "$template_prefix$taxonomy_prefix-{$current_language}-{$current_term->slug}.php";
				$templates[ ] = "$template_prefix$taxonomy_prefix-{$current_language}-{$term->term_id}.php";
				$templates[ ] = "$template_prefix$taxonomy_prefix-{$current_language}.php";
				$templates[ ] = "$template_prefix$taxonomy_prefix-{$current_term->slug}.php";
				$templates[ ] = "$template_prefix$taxonomy_prefix-{$term->term_id}.php";
			}
		}
		...
		if ( $original_term ) {
			$taxonomy_prefix = $taxonomy == 'post_tag' ? 'tag' : $taxonomy;
			$templates[ ] = "$template_prefix$taxonomy_prefix-{$current_language}-{$original_term->slug}.php";
			$templates[ ] = "$template_prefix$taxonomy_prefix-{$current_language}-{$original_term_id}.php";
			$templates[ ] = "$template_prefix$taxonomy_prefix-{$original_term->slug}.php";
			$templates[ ] = "$template_prefix$taxonomy_prefix-{$original_term_id}.php";
			$templates[ ] = "$template_prefix$taxonomy_prefix-{$current_language}.php";
			$templates[ ] = "$template_prefix$taxonomy_prefix.php";
		}
    ...
    function get_duplicates( $master_post_id ) {
        $this->post_duplication = $this->post_duplication === null ? new WPML_Post_Duplication()
