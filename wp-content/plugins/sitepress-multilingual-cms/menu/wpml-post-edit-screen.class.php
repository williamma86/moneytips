...
class WPML_Post_Edit_Screen {
	...
	public function __construct( $lang_negotiation_type ) {
		...
		if ( $lang_negotiation_type == 2 ) {
			add_filter( 'preview_post_link', array( $this, 'preview_post_link_filter' ), 10, 3 );
			add_filter( 'preview_page_link ', array( $this, 'preview_post_link_filter' ), 10, 3 );
		}
		add_action( 'icl_post_languages_options_after', array( $this, 'copy_from_original' ) );
	}

	public function preview_post_link_filter( $link, $post ) {
	...
	function copy_from_original() {
		...
		if ( $show && isset( $source_lang ) ) {
			$this->display_copy_from_button ( $source_lang, $source_lang_name, $post );
			$this->display_set_as_dupl_btn ( $post, $source_lang_name );
		}
	}

	private function display_copy_from_button( $source_lang, $source_lang_name, $post ) {
	...
	private function display_set_as_dupl_btn( $post, $source_lang_name ) {
