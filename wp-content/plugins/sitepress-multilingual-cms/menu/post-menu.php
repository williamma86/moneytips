...
$status_display = new WPML_Post_Status_Display( array( $post->ID ) );
...
<?php if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'edit' && $trid && $icl_can_translate_post ): ?>
			...
			<?php foreach($active_languages as $lang): if($selected_language==$lang['code']) continue; ?>
				<tr <?php if($oddev < 0): ?>class="icl_odd_row"<?php endif; ?>>
					<?php if(!isset($translations[$lang['code']]->element_id)):?>
						...
						<td align="right">
							<?php echo $status_display->get_status_html( $post->ID, $lang[ 'code' ] ); ?>
						</td>
						<td align="right">
							<?php
