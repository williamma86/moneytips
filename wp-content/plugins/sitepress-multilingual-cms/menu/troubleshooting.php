		...
		case 'icl_sync_jobs':
			...
			if ( !empty( $requests ) )
				foreach ( $requests as $request ) {
					$source_language = WPML_Pro_Translation::server_languages_map( $request[ 'language_name' ], true );
					$target_language = WPML_Pro_Translation::server_languages_map( $request[ 'target' ][ 'language_name' ], true );
	...
	jQuery(document).ready(function () {
		...
		function _icl_sync_cms_id(offset) {
			...
			jQuery.ajax({
				...
				dataType: 'json',
				success: function (msg) {
                    var cmsIdFix = jQuery('#icl_cms_id_fix');
					if (msg.errors > 0) {
						alert(msg.message);
                        cmsIdFix.removeAttr('disabled');
                        cmsIdFix.next().fadeOut();
		...
		jQuery('#icl_sync_cancelled').click(function () {
			...
			jQuery.ajax({
				...
				success: function (msg) {
                    ...
                    var syncCancelled = jQuery('#icl_sync_cancelled');
                    syncCancelled.removeAttr('disabled');
                    syncCancelled.next().fadeOut();
				}
			});
		});
		...
		jQuery(document).delegate('#icl_ts_cancel_ok', 'click', function () {
			...
			jQuery.ajax({
				...
				success: function (msg) {
                    ...
                    var tsCancelOk = jQuery('#icl_ts_cancel_ok');
                    tsCancelOk.removeAttr('disabled');
                    tsCancelOk.next().fadeOut();
				}
			});
			return false;
		});
	...
	<div class="icl_cyan_box">
		...
		<?php } else { ?>
			...
			echo '<form method="post" onsubmit="return confirm(\'' . __(
					'Are you sure you want to reset all languages data? This operation cannot be reversed.',
					'sitepress'
				) . '\')">';
			wp_nonce_field( 'icl_reset_all', 'icl_reset_allnonce' );
			echo '<p class="error" style="padding:6px;">';
			_e(	"All translations you have sent to remote translation services will be lost if you reset WPML's data. They cannot be recovered later.", 'sitepress' );
			echo '</p>';
			...
			echo '<input id="reset-all-but" type="submit" disabled="disabled" class="button-primary" value="' . __(
					'Reset all language data and deactivate WPML',
					'sitepress'
				) . '" />';
			echo '</form>';
			?>
