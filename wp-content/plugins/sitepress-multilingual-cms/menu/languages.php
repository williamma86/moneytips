                    ...
                    <div class="wpml-section-content">
                        ...
                        <form id="icl_save_language_negotiation_type" name="icl_save_language_negotiation_type" action="">
                            ...
                            <ul>
                                ...
                                $client = new WP_Http();
                                if ( empty( $_POST[ 'url' ] ) || false === strpos ( $_POST[ 'url' ], '?' ) ) {
                                    $url_glue = '?';
                                } else {
                                    $url_glue = '&';
                                }
                                $abs_home = $wpml_url_converter->get_abs_home ();
                                $sample_lang_url = trailingslashit ( $abs_home ) . $sample_lang[ 'code' ] . '/';
                                $response        = $client->request (
                                    $sample_lang_url . '/' . $url_glue . '____icl_validate_domain=1',
                                    array( 'timeout' => 15, 'decompress' => false )
                                );
                                if ( !is_wp_error (
                                        $response
                                    ) && ( $response[ 'response' ][ 'code' ] == '200' ) && ( $response[ 'body' ] === '<!--' . $sample_lang_url . '-->' )
                                ) {
                                    $icl_folder_url_disabled = false;
                                } elseif ( is_wp_error (
                                               $response
                                           ) && isset( $response->errors[ 'http_request_failed' ] ) && $response->errors[ 'http_request_failed' ][ 0 ] == 'SSL certificate problem: self signed certificate'
                                ) {
                                    $icl_folder_url_disabled = false;
                                } else {
                                ...
                                <li>
                                    <label>
                                        ...
                                        (<?php
											echo sprintf( '%s%s - %s, %s - %s', trailingslashit( $abs_home ), empty( $setting_urls[ 'directory_for_default_language' ] ) ? '' : trailingslashit( $default_language ), $default_language_details[ 'display_name' ], trailingslashit( $sample_lang_url ), $sample_lang[ 'display_name' ] );
											?>)
                                        </span>
                                    </label>
	                                ...
	                                <?php if ( $icl_folder_url_disabled || trailingslashit( $abs_home ) != trailingslashit( get_site_url() ) ): ?>
