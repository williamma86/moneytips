...
class WPML_Slug_Filter {
	...
	public function pre_term_slug_filter( $slug, $taxonomy ) {
		if ( ( isset( $_REQUEST[ 'tag-name' ] ) || isset( $_REQUEST[ 'name' ] ) )
			...
			$slug = $slug !== '' ? urlencode ( WPML_Terms_Translations::term_unique_slug ( $slug, $taxonomy, $lang ) ) : $slug;
		}
	...
	function wp_unique_post_slug( $slug_suggested, $post_id, $post_status, $post_type, $post_parent, $slug ) {
		...
		if ( $post_status !== 'auto-draft' && $sitepress->is_translated_post_type ( $post_type ) && $post_id ) {
			...
			if($slug_suggested_wpml !== $slug_suggested){
	...
	private function post_slug_exists( $post_id, $post_language, $slug, $parent = false ) {
		global $wpdb;
		...
		$post_name_check_sql	  = "	SELECT p.post_name
										...
										WHERE p.post_name = %s
											...
											{$parent_snippet}
										LIMIT 1";
		$post_name_check_prepared = $wpdb->prepare ( $post_name_check_sql, $slug, $post_id, $post_language );
	...
	private function find_unique_slug_post( $post_id, $post_type, $post_language, $post_parent, $slug ) {
		...
		if ( $this->post_slug_exists ( $post_id, $post_language, $slug, $post_parent )
			...
			do {
				...
				$suffix ++;
			} while ( $this->post_slug_exists ( $post_id, $post_language, $alt_post_name, $post_parent ) );
			$slug = $alt_post_name;
		}
