...
function wpml_site_uses_icl_message_notice() {
	...
	$downgrade_url            = '#'; //TODO: [WPML 3.2] @sarah must provide the final url before releasing 3.2.
...
function wpml_plugin_just_activated() {
	$activate_action      = isset( $_REQUEST[ 'action' ] )
	                        && ( $_REQUEST[ 'action' ] === 'activate'
	                             || $_REQUEST[ 'action' ] === 'activate-selected' );
	$wpml_plugin_selected = isset( $_REQUEST[ 'plugin' ] )
	                        && $_REQUEST[ 'plugin' ] === ICL_PLUGIN_FULL_PATH
	                        || isset( $_REQUEST[ 'checked' ] )
	                           && is_array ( $_REQUEST[ 'checked' ] )
	                           && in_array ( ICL_PLUGIN_FOLDER . '/' . ICL_PLUGIN_FILE, $_REQUEST[ 'checked' ], true );

	return $activate_action || $wpml_plugin_selected;
}
