...
class ICLMenusSync
{
	...
	function do_sync( $data )
	{
				...
				foreach ( $translations as $language => $name ) {
					...
					do {
						$new_menu_id = wp_update_nav_menu_object( 0,
						                                          array( 'menu-name' => $name . $menu_indentation . ( $menu_increment ? $menu_increment : '' ) ) );
		...
		if ( !empty( $data[ 'options_changed' ] ) ) {
			foreach ( $data[ 'options_changed' ] as $menu_id => $translations ) {
		...
		wp_redirect( admin_url( 'admin.php?page=' . ICL_PLUGIN_FOLDER . '/menu/menu-sync/menus-sync.php&updated=1' ) );
		exit;
	}
	...
	function render_items_tree_default( $menu_id, $parent = 0, $depth = 0 )
	{
						...
						<td>
							...
							if ( !empty( $item_translation[ 'ID' ] ) ) {
								...
								if ( $item_translation[ 'menu_order' ] != $item_translation[ 'menu_order_new' ] || $item_translation[ 'depth' ] != $item[ 'depth' ] ) { // MOVED
									...
									$this->index_changed('label_missing', $item_id, $item_translation['title'], $menu_id, $lang_code);
								} elseif ( $item_translation[ 'label_changed' ] ) {
                                    $this->index_changed('label_changed', $item_id, $item_translation['title'], $menu_id, $lang_code);
								} elseif ( $item_translation[ 'url_missing' ] ) {
                                    $this->index_changed('url_missing', $item_id, $item_translation['url'], $menu_id, $lang_code);
								} elseif ( $item_translation[ 'url_changed' ] ) {
                                    $this->index_changed('url_changed', $item_id, $item_translation['url'], $menu_id, $lang_code);
								} else { // NO CHANGE
                                    $need_sync--;
        ...
        $this->index_changed ( 'options_changed', 'auto_add', $menu_options[ 'auto_add' ], $menu_id, $lang_code, $change );
        }
        ?></td><?php
