...
class iclNavMenu{
    ...
    function __construct(){
        ...
        if(is_admin()){
            ...
            add_filter('wp_get_nav_menus', array($this, 'wp_get_nav_menus_filter'), 10, 2);
    ...
    function init(){
        ...
        } elseif ( (bool) ( $lang = $wpml_request_handler->get_cookie_lang () ) !== false ) {
    ...
    */
    ...
    function nav_menu_language_controls(){
        global $sitepress, $wpdb;
		$default_language = $sitepress->get_default_language();
        if ( isset( $this->current_menu[ 'language' ] ) && $this->current_menu[ 'language' ] != $default_language ) {
    ...
    function _enable_sitepress_query_filters($args){
		...
		function wp_get_nav_menus_filter($menus, $args) {
			global $pagenow;
		...
		private function unfilter_non_default_language_menus($menus) {
			...
			foreach ($menus as $index => $menu) {
				$menu_language = $this->get_menu_language($menu);
		 ...
		 * @param int|object $menu menu object or menu id
		 * @return null|string language code for menu or null
		 *
		 */
		function get_menu_language($menu) {
			if (isset($menu->term_taxonomy_id)) {
				$menu = $menu->term_taxonomy_id;
			}
			$language = null;
			
			if ( is_numeric($menu) ) {
				global $sitepress;
				$language = $sitepress->get_language_for_element($menu, 'tax_nav_menu');
			}
