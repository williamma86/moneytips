...
class WPML_Post_Synchronization{
	 ...
	 * @param WPML_Admin_Post_Actions $post_translations
	 */
	public function __construct(&$settings, &$post_translations){
		...
		$this->language_order      = isset( $settings[ 'languages_order' ] ) ? $settings[ 'languages_order' ] : array();
	...
	function fix_translated_parent( $original_id, $translated_id, $translation_language ) {
		...
		$original_parent = wp_get_post_parent_id($original_id);
		$translated_parent = $wpml_post_translations->element_id_in($original_parent, $translation_language);
		if($translated_parent){
			$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->posts}
										   SET post_parent=%d
										   WHERE ID = %d",
			                              array( $translated_parent, $translated_id ) ) );
		}
	}

	public function deleted_post_actions( $post_id, $keep_db_entries = false ) {

		$post_translations = $this->post_translations;
		...
		$trid = $post_translations->get_element_trid( $post_id );
		$translated_ids = $post_translations->get_element_translations( $post_id, $trid, true );
		$lang_code      = $post_translations->get_element_lang_code( $post_id );

		if ( $this->sync_delete && ! empty( $translated_ids ) ) {
			foreach ( $translated_ids as $trans_id ) {
				if ( $keep_db_entries ) {
					remove_action( 'wp_trash_post', array( $this, 'trashed_post_actions' ) );
					wp_trash_post( $trans_id );
					add_action( 'wp_trash_post', array( $this, 'trashed_post_actions' ) );
				} else {
					wp_delete_post( $trans_id );
				}
			}
		}

		if ( ! $keep_db_entries ) {
			$post_translations->delete_post_translation_entry( $post_id );
	...
	function untrashed_post_actions( $post_id ) {
		$post_translations = $this->post_translations;
		if ( $this->sync_delete ) {
			$translations = $post_translations->get_element_translations( $post_id, false, true );
			foreach ( $translations as $t_id ) {
				wp_untrash_post( $t_id );
			}
		}
		$post_type = get_post_type( $post_id );
		require_once ICL_PLUGIN_PATH . '/inc/cache.php';
	...
	public function sync_with_translations( $post_id ) {
		global $wpdb;
		$post           = get_post ( $post_id );
		$translated_ids = $this->post_translations->get_element_translations ( $post_id, false, true );
		...
		$post_status    = $this->sync_private_flag ? get_post_status ( $post_id ) : null;
		...
		$page_template  = $this->sync_page_template && get_post_type ( $post_id ) === 'page'
			? get_page_template_slug ( $post_id ) : false;
		$post_date      = $this->sync_post_date
			? $wpdb->get_var ( $wpdb->prepare ( "SELECT post_date FROM {$wpdb->posts} WHERE ID=%d", $post_id ) ) : null;
		...
		foreach ( $translated_ids as $lang_code => $translated_pid ) {
			...
			if ( $this->sync_parent ) {
				$this->fix_translated_parent ( $post_id, $translated_pid, $lang_code );
			}
	...
	public function sync_sticky_flag($trid, $post_vars){
		global $sitepress;

		if ( $this->sync_sticky_flag && isset( $post_vars[ 'post_type' ] )
			...
			$translations = $this->post_translations->get_element_translations ( false, $trid, false );
			...
			add_filter( 'pre_option_sticky_posts', array( $sitepress, 'option_sticky_posts' ) ); // add filter back
		}
	}
	...
	private function sync_custom_fields( $original_id, $post_id ) {
		...
		if ( $original_id && $original_id != $post_id ) {
			$sitepress->copy_custom_fields ( $original_id, $post_id );
		} else {
			$translations = $this->post_translations->get_element_translations ( $post_id, false, true );
			foreach ( $translations as $t_id ) {
				$sitepress->copy_custom_fields ( $post_id, $t_id );
			}
		}
	}
