...
class ICL_AdminNotifier {
	...
	public static function remove_message_group( $message_group ) {
		...
		if(!isset($all_messages['messages'])) return;
	...
	static function troubleshooting() {
		...
		if ( self::has_hidden_messages() ) {
			...
			<p>
				...
				<br />
				<br />
				<input id="icl_restore_notifications_all_users"
				       name="icl_restore_notifications_all_users"
				       type="checkbox"
				       value="1" /><label for="icl_restore_notifications_all_users"><?php echo __( 'Apply to all users', 'sitepress' ); ?></label>
				<br />
				<br />
