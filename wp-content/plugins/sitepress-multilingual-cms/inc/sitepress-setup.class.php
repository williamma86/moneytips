...
class SitePress_Setup {
	...
	static function fill_languages() {
		...
		if ( !self::languages_table_is_complete() ) {
			...
			$active_languages = $sitepress !== null ? $sitepress->get_active_languages() : array();
	...
	static function fill_languages_translations() {
		...
		if ( !self::languages_table_is_complete() ) {
			...
			if ( $truncate_result ) {
				...
				// Build a sql statement to insert all the data in one go.
				
				$sql = "INSERT INTO {$table_name} (id, language_code, display_language_code, name) VALUES ";
				$index = 1;
				...
				foreach ( $languages_names as $lang => $val ) {
					...
					foreach ( $val[ 'tr' ] as $k => $display ) {
						...
						$sql .= '(' . $index . ",'" . $languages_codes[ $lang ] . "','" . $languages_codes[ $k ] . "','" . $display . "')";
						$index++;
					}
				}
				$sql .= ";";
