 ...
 * @param string $element_type               Use comment, post, page, {custom post type name}, nav_menu, nav_menu_item, category, post_tag, etc.
 ...
 * @deprecated Use 'translate_object_id' filter instead
 */
function icl_object_id( $element_id, $element_type = 'post', $return_original_if_missing = false, $ulanguage_code = null ) {
    ...
    if ( is_null ( $ulanguage_code ) ) {
        $ulanguage_code = $ulanguage_code ? $ulanguage_code : $sitepress->get_current_language ();
    }

    if ( isset( $wp_taxonomies[ $element_type ] ) ) {
        /** @var WPML_Term_Translation $wpml_term_translations */
        global $wpml_term_translations;
        $ret_element_id = $wpml_term_translations->term_id_in ( $element_id, $ulanguage_code );
    } elseif ( isset( $wp_post_types[ $element_type ] ) ) {
        /** @var WPML_Post_Translation $wpml_post_translations */
        global $wpml_post_translations;
        $ret_element_id = $wpml_post_translations->element_id_in ( $element_id, $ulanguage_code );
    } else {
        $ret_element_id = null;
    }

    $ret_element_id = $ret_element_id
        ? $ret_element_id : ( $return_original_if_missing && !$ret_element_id ? $element_id : null );
...
/**
 * @since 3.1.6
 * @deprecated @since 3.2: use 'wpml_translate_object_id' with the same arguments
 */
 ...
 * @since 3.2
 */
add_filter('wpml_translate_object_id', 'icl_object_id', 10, 4);
...
function icl_get_current_language() {
	return apply_filters('wmpl_get_current_language', '');
}
...
function wpml_cf_translation_preferences( $id, $custom_field = false, $class = 'wpml', $ajax = false, $default_value = 'ignore', $fieldset = false, $suppress_error = false ) {
	...
	if ( $fieldset ) {
		$output .= '
<fieldset id="wpml_cf_translation_preferences_fieldset_' . $id . '" class="wpml_cf_translation_preferences_fieldset ' . $class . '-form-fieldset form-fieldset fieldset">' . '<legend>' . __( 'Translation preferences', 'wpml' ) . '</legend>';
	}
	...
	if ( $custom_field ) {
		if ( defined( 'WPML_TM_VERSION' ) && ! empty( $iclTranslationManagement ) ) {
			...
			$output .= '<span style="color:#FF0000;">' . __( "To synchronize values for translations, you need to enable WPML's Translation Management module.", 'wpml' ) . '</span>';
			$disabled = true;
		}
	} else if ( ! $suppress_error ) {
		$output .= '<span style="color:#FF0000;">' . __( 'Error: Something is wrong with field value. Translation preferences can not be set.', 'wpml' ) . '</span>';
		$disabled = true;
	}
	//$disabled = !empty($disabled) ? ' readonly="readonly" disabled="disabled"' : '';
	$output .= '<div class="description ' . $class . '-form-description ' . $class . '-form-description-fieldset description-fieldset">' . __( 'Choose what to do when translating content with this field:', 'wpml' ) . '</div>';
	...
	$output .= '<ul><li>';
	$output .= wpml_translation_preference_input_helper($args, 'wpml_cf_translation_preferences_option_ignore_', '0', __( 'Do nothing', 'wpml' ));
	$output .= '</li><li>';
	$output .= wpml_translation_preference_input_helper($args, 'wpml_cf_translation_preferences_option_copy_', '1', __( 'Copy from original', 'wpml' ));
	$output .= '</li><li>';
	$output .= wpml_translation_preference_input_helper($args, 'wpml_cf_translation_preferences_option_translate_', '2', __( 'Translate', 'wpml' ));
	$output .= '</li></ul>';
...
/**
 * Accepts and ID of an element and returns its translation type.
 ...
 *
 * @param int    $id   The ID of the element from which to get translation information
 * @param string $type The type of the element (post, category, etc.)
 *
 * @return int
 */
