...
class TM_Notification{
	...
	private static $_process_mail_queue = false;

	private static $mail_cache = array();

	function __construct() {
		add_action( 'init', array( __CLASS__, 'mail_queue' ), 9999 );
		//See wpmltm-174
		//add_action( 'wp_redirect', array( __CLASS__, 'mail_queue' ), 9999 );
		add_action( 'icl_ajx_custom_call', array( __CLASS__, 'mail_queue' ), 9999 );
	}
	...
	function new_job_translator($job_id, $translator_id){
		...
		if ( !isset( $job->manager_id ) || !isset( $job->translator_id ) || $job->manager_id == $job->translator_id ) {
			return;
		}
	...
	private static function send_mail($mail, $language = false){
		global $sitepress;
		...
		if ($mail != 'empty_queue') {
			self::$mail_cache[$mail['type']][$mail['to']][$mail['subject']]['body'][] = $mail['body'];
			if (isset($mail['attachment'])) {
				self::$mail_cache[$mail['type']][$mail['to']][$mail['subject']]['attachment'][] = $mail['attachment'];
			}
			self::$_process_mail_queue = TRUE;
		} else if (!empty(self::$mail_cache)) {
			$tj_url = admin_url('admin.php?page=' . WPML_TM_FOLDER . '/menu/translations-queue.php');
			foreach (self::$mail_cache as $type => $mail_to_send) {
			...
			self::$mail_cache = array();
			self::$_process_mail_queue = FALSE;
		}
	...
	public static function mail_queue() {
		if (self::$_process_mail_queue && count(self::$mail_cache) > 0 ) {
			self::send_mail('empty_queue');
		}
	}
}
