...
class WPML_Config
{
	...
	private static function sync_settings($config, $index_singular, $index_plural, $read_only_index){
		global $sitepress, $iclTranslationManagement;

		if ( !empty( $config[ 'wpml-config' ][ $index_plural ] ) ) {
			$sync_index      = $index_plural . '_sync_option';
			$sync_option     = $sitepress->get_setting ( $sync_index, array() );
			if ( !is_numeric ( key ( current ( $config[ 'wpml-config' ][ $index_plural ] ) ) ) ) {
				$cf[ 0 ] = $config[ 'wpml-config' ][ $index_plural ][ $index_singular ];
			} else {
				$cf = $config[ 'wpml-config' ][ $index_plural ][ $index_singular ];
			}
			...
			add_filter (
				'get_translatable_' . $index_plural,
				array( $iclTranslationManagement, '_override_get_translatable_' . $index_plural )
			);
		}
	}

	private static function update_tm_settings( $config, $index_singular, $index_plural ) {
		global $iclTranslationManagement;
		$read_only_index = $index_plural . '_readonly_config';
		self::sync_settings($config, $index_singular, $index_plural, $read_only_index);

		// taxonomies - check what's been removed
		if ( !empty( $iclTranslationManagement->settings[ $read_only_index ] ) ) {
			$config_values = array();
			foreach ( $config['wpml-config'][$index_plural][$index_singular] as $config_value ) {
				$config_values[ $config_value[ 'value' ] ] = $config_value[ 'attr' ][ 'translate' ];
			}
			foreach ( $iclTranslationManagement->settings[ $read_only_index ] as $tconf => $tconf_val ) {
				if ( !isset( $config_values[ $tconf ] ) ) {
					unset( $iclTranslationManagement->settings[ $read_only_index ][ $tconf ] );
				}
			}
